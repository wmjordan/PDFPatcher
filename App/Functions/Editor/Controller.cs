...
internal sealed class Controller
{
	...
	private HashSet<XmlNode> ProcessBookmarks(IList si, HashSet<XmlElement> processedItems, bool includeDescendant,
		IPdfInfoXmlProcessor processor) {
	...
	private static void ProcessItem(bool includeDescendant, IPdfInfoXmlProcessor processor,
	...
	private void _LoadBookmarkWorker_DoWork(object sender, DoWorkEventArgs e) {
		...
		using (PdfReader reader = PdfHelper.OpenPdfFile(path, AppContext.LoadPartialPdfFile, false)) {
			try {
				...
				e.Result = new object[] {
					OutlineManager.GetBookmark(reader, new UnitConverter() {Unit = Constants.Units.Point}), importMode,
					path
				};
	...
	private void ImportBookmarks(BookmarkEditorView editView, XmlNodeList bookmarks) {
		if (Model.Document == null) {
			Model.Document = new PdfInfoXmlDocument();
		}
	...
	internal EditModel.Region CopyText(Point cp, PagePosition pp) {
		...
		if (Model.InsertBookmarkWithOcrOnly == false && lines.HasContent()) {
			...
			foreach (MuTextLine line in lines) {
				if (sb.Length > 100) {
					break;
				}
	...
	internal void LabelAtPage(PagePosition position) {
		...
		PdfViewerControl v = View.Viewer;
		InsertPageLabelForm f = new() { Location = Cursor.Position.Transpose(-16, -16), PageNumber = position.Page };
	...
	internal void MergeBookmark(IList<BookmarkElement> es) {
		...
		for (int i = 1; i < l; i++) {
			...
			while (es[i].HasChildNodes) {
				XmlElement c = es[i].FirstChild as XmlElement;
				if (c == null) {
					continue;
				}
	...
	private static BookmarkContainer CreateNewSiblingBookmarkForParent(BookmarkContainer bm,
		List<MuTextSpan> spans) {
		TrimBookmarkText(bm);
	...
	private static BookmarkContainer CreateNewSiblingBookmark(BookmarkContainer bm, List<MuTextSpan> spans) {
		TrimBookmarkText(bm);
