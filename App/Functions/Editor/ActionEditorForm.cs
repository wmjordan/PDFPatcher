...
public partial class ActionEditorForm : Form
{
	...
	public ActionEditorForm(BookmarkElement element) {
		...
		i = Array.IndexOf(Constants.DestinationAttributes.ViewType.Names,
			element.GetAttribute(Constants.DestinationAttributes.View));
		...
		_NewWindowBox.Checked =
			element.GetAttribute(Constants.DestinationAttributes.NewWindow) == Constants.Boolean.True;
		...
		if (_WidthBox.Enabled) {
			decimal v = _WidthBox.Value - _LeftBox.Value;
		...
		if (_HeightBox.Enabled) {
			decimal v = _TopBox.Value - _HeightBox.Value;
	...
	private static void InitCoordinateValue(XmlElement element, string name, NumericUpDown control, CheckBox check) {
	...
	private void _OkButton_Click(object source, EventArgs args) {
		...
		string act = _ActionBox.SelectedItem as string;
		if (act == NoAction) {
			act = null;
		}
		...
		if (_PathPanel.Enabled) {
			...
			if (_NewWindowBox.Enabled) {
				SetValue(Constants.DestinationAttributes.NewWindow,
					_NewWindowBox.Checked ? Constants.Boolean.True : Constants.Boolean.False);
			}
		}
	...
	private void Control_ValueChanged(object sender, EventArgs e) {
		if (sender == _ActionBox) {
			string i = _ActionBox.SelectedItem as string;
			...
			switch (_ZoomRateBox.Text) {
				...
				case Constants.DestinationAttributes.ViewType.FitB:
					_TopBox.Enabled = _LeftBox.Enabled =
						_KeepXBox.Enabled = _KeepYBox.Enabled = _RectanglePanel.Enabled = false;
					break;
