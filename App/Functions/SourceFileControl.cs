...
namespace PDFPatcher
{
	public partial class SourceFileControl : UserControl
	{
		...
		/// 获取选定的 PDF 文件列表的第一项。
		/// </summary>
		internal string FirstFile {
			get {
				return this.Files != null && this.Files.Length > 0 ? this.Files[0] : String.Empty;
			}
		}
		...
		public override string Text {
			get {
				return _SourcePdfBox.Text;
			}
			set {
				_SourcePdfBox.Text = value;
			}
		}
		...
		private void _BrowseSourcePdfButton_Click (object sender, EventArgs e) {
			...
			if (_OpenPdfBox.ShowDialog () == DialogResult.OK) {
				...
				if (this.BrowseSelectedFiles != null) {
					this.BrowseSelectedFiles (sender, e);
				}
			}
		}
		...
		private void SelectFiles (string[] files) {
			var t = _SourcePdfBox.Text;
			if (files.Length > 1) {
				this.Text = String.Concat ("<选定了 ", files.Length, " 个文件>", System.IO.Path.GetDirectoryName (files[0]));
			}
			else if (files[0] != t) {
				this.Text = files[0];
			}
		...
		private void SourceFileControl_Show (object sender, EventArgs e) {
			...
			if (this.Visible && AppContext.MainForm != null) {
				this._SourcePdfBox.Contents = AppContext.Recent.SourcePdfFiles;
			}
