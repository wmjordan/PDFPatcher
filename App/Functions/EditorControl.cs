...
public sealed partial class EditorControl : FunctionControl, IDocumentEditor, IEditView
{
	...
	private static CommandRegistry<Controller> InitCommands() {
		...
		d.Register(new BookmarkStyleCommand(SetTextStyleProcessor.Style.SetBold),
			Commands.EditorBookmarkBold);
		d.Register(new BookmarkStyleCommand(SetTextStyleProcessor.Style.SetItalic),
		...
		d.Register(new SimpleBookmarkCommand<BookmarkOpenStatusProcessor, bool>(false),
			"_SetOpenStatusFalse");
		foreach (string item in Constants.DestinationAttributes.ViewType.Names) {
			d.Register(new BookmarkActionCommand(item), item);
		}

		d.Register(new BookmarkActionCommand(Constants.Coordinates.Unchanged),
	...
	protected override void OnLoad(EventArgs e) {
		...
		_ChangeCase.DropDownItemClicked += (object s, ToolStripItemClickedEventArgs args) => {
			...
			_EditMenu.Hide();
			int i = Array.IndexOf(SetTitleCaseProcessor.CaseNames, args.ClickedItem.Text);
			if (i != -1) {
