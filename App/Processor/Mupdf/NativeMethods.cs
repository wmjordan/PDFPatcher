...
internal static partial class NativeMethods
{
	...
	private static extern ContextHandle NewContext(IntPtr alloc, IntPtr locks, uint max_store,
	...
	/// <param name="list">
	///     A display list, created by fz_new_display_list and populated with objects from a page by running
	///     fz_run_page on a device obtained from fz_new_list_device.
	/// </param>
	/// <param name="dev">The target device to render the display list.</param>
	/// <param name="ctm">
	///     Transform to apply to display list contents. May include for example scaling and rotation, see
	///     fz_scale, fz_rotate and fz_concat.Set to fz_identity if no transformation is desired.
	/// </param>
	/// <param name="scissor">
	///     Only the part of the contents of the display list visible within this area will be considered
	///     when the list is run through the device.This does not imply for tile objects contained in the display list.
	/// </param>
	/// <param name="cookie">
	///     Communication mechanism between caller and library running the page.Intended for multi-threaded
	///     applications, while single-threaded applications set cookie to NULL.The caller may abort an ongoing page run.Cookie
	///     also communicates progress information back to the caller.The fields inside cookie are continually updated while
	...
	public static extern void RunPageWidgets(ContextHandle context, PageHandle page, DeviceHandle dev, Matrix transform,
		ref MuCookie cookie);
