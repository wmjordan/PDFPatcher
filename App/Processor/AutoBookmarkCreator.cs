...
namespace PDFPatcher.Processor
{
	sealed class AutoBookmarkCreator
	{
		...
		{
			float _fontSizeThreshold;
			//Rectangle _positionRectangle;
			bool _mergeAdjacentTitles;
			bool _mergeDifferentSizeTitles;
			float _textWidth, _charWidth;
			List<TextInfo> _TextList;
			LevelProcessor _levelProcessor;
			AutoBookmarkContext _context;
			const string __AddSpaceAfterCharacters = ":.,\"'?!)]};";
			const string __InsertSpaceBeforeCharacters = "\"'([{";
			...
			/// 获取页面内容的文本。
			/// </summary>
			internal List<TextInfo> TextList {
				get { return _TextList; }
			}
			/// <summary>
			/// 获取字体列表。
			/// </summary>
			internal IDictionary<int,string> FontList {
				get { return base.Fonts; }
			}
