...
internal sealed class AutoBookmarkCreator
{
			...
			foreach (MatchPattern item in options.IgnorePatterns) {
				...
				try {
					...
					Tracker.TraceMessage(Tracker.Category.Alert,
						string.Concat("忽略文本（", item, "）无效：", ex.Message));
				}
			}
		}
				...
				if (tr != null && tr.Count > 0) {
					...
					foreach (TextRegion item in tr) {
						...
						if (options.AutoHierarchicalArrangement) {
							...
							be = doc.DocumentElement.AppendChild(doc.CreateElement(Constants.Bookmark)) as
								XmlElement;
						}
		...
		if (options.PageTopForLevel > 0) {
			XmlNodeList topics =
				doc.DocumentElement.SelectNodes(
					".//书签[count(ancestor::书签) < " + (options.PageTopForLevel + 1) + "]");
	...
	private static void WriteDocumentFontOccurances(XmlWriter writer, AutoBookmarkOptions options,
		...
		foreach (KeyValuePair<int, string> item in p.FontList) {
			string fo = "0";
			List<SizeOccurance> sl = fontOccurances.GetOccurance(item.Value);
			if (sl != null) {
				if (dl.Contains(item.Value) == false) {
					int o = 0;
					foreach (SizeOccurance s in sl) {
						o += s.Occurance;
					}
			...
			if (sl != null) {
				foreach (SizeOccurance s in sl) {
					...
					if (options.DisplayFontStatistics && (s.Occurance > 0 || options.DisplayAllFonts)) {
						Tracker.TraceMessage(string.Concat("\t尺寸：", s.Size.ToText(), "\t出现次数：",
							s.Occurance.ToText(), "\t首次出现于第", s.FirstPage.ToText(), "页（", s.FirstInstance, "）"));
					}
	...
	private string ConcatRegionText(TextRegion region) {
