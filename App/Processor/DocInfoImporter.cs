...
namespace PDFPatcher.Processor
{
	internal sealed class DocInfoImporter
	{
		...
		internal DocInfoImporter(ImporterOptions importerOptions, PdfReader pdf, PatcherOptions patcherOptions,
			BookmarkRootElement bookmarkRoot) {
		...
		internal PdfPageLabels ImportPageLabels() {
			...
			foreach (XmlElement item in pn) {
				if (item.GetAttribute(Constants.PageLabelsAttributes.PageNumber).TryParse(out int physicalPage) ==
				    false || physicalPage < 1) {
					Trace.WriteLine(string.Concat("在“", Constants.PageLabels, "”的“",
						Constants.PageLabelsAttributes.Style, "”元素中，必须指定大于或等于 1 的“",
						Constants.PageLabelsAttributes.PageNumber, "”属性。"));
					continue;
				}
			...
			foreach (var item in labels) {
				if (item.PageNumber > 0 && item.StartPage > 0) {
					pls.AddPageLabel(item.PageNumber,
						ValueHelper.MapValue(item.Style, Constants.PageLabelStyles.Names,
							Constants.PageLabelStyles.Values, PdfPageLabels.DECIMAL_ARABIC_NUMERALS),
						item.Prefix,
						item.StartPage
					);
					i++;
		...
		internal void ImportPageLinks(PdfReader r, PdfStamper w) {
			...
			var ls = _infoDoc.DocumentElement.SelectNodes(Constants.PageLink + "/" + Constants.PageLinkAttributes.Link +
		...
		internal static void ImportAction(PdfWriter writer, PdfDictionary dict, XmlElement map, int maxPageNumber,
			...
			switch (action) {
				case Constants.ActionType.Goto:
					...
					else if (string.IsNullOrEmpty(p = map.GetAttribute(Constants.DestinationAttributes.Page)) ==
					         false) {
						var ar = new PdfArray();
		...
		private static void CreateDestination(PdfWriter writer, XmlElement map, string p, PdfArray ar, bool isRemote) {
			...
			switch (p = map.GetAttribute(Constants.DestinationAttributes.View)) {
				...
				default:
					...
					if (map.GetAttribute(Constants.Coordinates.ScaleFactor).TryParse(out pos[2]) == false ||
					    pos[2] < 0) {
						pos[2] = float.NaN;
					}
		...
		internal static void OverrideViewerPreferences(ViewerOptions options, PdfReader reader, PdfWriter writer) {
			var v = ValueHelper.MapValue(options.InitialView, Constants.PageLayoutType.Names,
				Constants.PageLayoutType.PdfNames, PdfName.NONE);
			if (PdfName.NONE.Equals(v) == false) {
		...
		internal void ImportPageSettings(PdfReader pdf) {
			...
			var ps = _infoDoc.DocumentElement.SelectNodes(string.Concat(Constants.Content.PageSettings.ThisName, "/",
