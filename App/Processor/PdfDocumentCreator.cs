...
{
	internal static readonly string[] SupportedFileTypes = [".pdf", ".tif", ".jpg", ".gif", ".png", ".tiff", ".bmp", ".jpeg", ".jp2", ".j2k"];
	...
	static readonly PixelFormat[] __JpgFormats = [
	...
	static iTextImage LoadImage(SourceItem sourceFile, string ext) {
		...
		if (imageItem == null || !cropOptions.NeedCropping) {
				...
				&& (// 页面不足以放下当前尺寸的图片
					(image.ScaledHeight > _content.Height || image.ScaledWidth > _content.Width)
						&& (image.ScaledWidth > image.ScaledHeight && _portrait == true
							|| image.ScaledHeight > image.ScaledWidth && !_portrait)
					||
					// 图片较小，可以还原为原始的页面方向
	...
	static Image LoadImageFrame(SourceItem.Image source, bool recompressWithJbig2, ref FreeImageBitmap fi) {
		...
		else if (fi.PixelFormat == PixelFormat.Format1bppIndexed && fi.ImageFormat != FREE_IMAGE_FORMAT.FIF_TIFF && !recompressWithJbig2) {
