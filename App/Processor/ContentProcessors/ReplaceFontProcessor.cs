...
internal sealed class ReplaceFontProcessor : IPageProcessor
{
	...
	private static PdfString RewriteText(NewFont newFont, TrueTypeFontUnicode ttf, string text) {
	...
	private static bool DetectLegacyCjkFont(PdfDictionary font) {
		...
		if (en == null
			|| (PdfName.WIN_ANSI_ENCODING.Equals(en) || __GbkEncoding.Equals(en) || __GbEncoding.Equals(en)) ==
			false
		   ) {
			return false;
		}
	...
	private void LoadFonts(PageProcessorContext context, PdfDictionary fonts) {
		Dictionary<PdfName, PRIndirectReference>
			r = new(fonts.Length); // 替代的字体
		foreach (KeyValuePair<PdfName, PdfObject> item in fonts) {
			string sn; // 替换字体名称
			string n; // 字体名称
			PdfIndirectReference fr = item.Value as PdfIndirectReference;
			if (fr == null
	...
	private static void ReadSingleByteFontWidths(PdfDictionary font, FontInfo fontInfo, NewFont newfont) {
		...
		foreach (KeyValuePair<string, NewFont> font in _newFonts) {
			...
			if (newFont.AbsentChars.Count > 0) {
				Tracker.TraceMessage(Tracker.Category.ImportantMessage,
					string.Concat("丢失", newFont.AbsentChars.Count, "字：",
	...
	private static PdfStream SubsetFont(NewFont ef, TrueTypeFontUnicode ttf) {
