...
partial class PdfActionExporter
{
	...
	internal void ExportAction(Document pdf, PdfDictionary action, PdfDictionary names, XmlWriter target) {
		...
		switch (((action[PdfNames.S] as PdfName)?.PredefinedValue ?? PdfNames.GoTo)) {
			case PdfNames.GoTo:
	...
	internal void ExportFileLocation(XmlWriter w, PdfObject file) {
		switch (file.TypeKind) {
			case Kind.Dictionary:
				...
				break;
			case Kind.String:
				w.WriteAttributeString(Constants.DestinationAttributes.Path, PdfHelper.GetValidXmlString(((PdfString)file).Decode(AppContext.Encodings.BookmarkEncoding)));
				break;
		}
	}
