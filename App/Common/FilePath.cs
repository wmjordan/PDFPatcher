...
public readonly struct FilePath : IEquatable<FilePath>
{
	...
	public static readonly FilePath AppRoot =
		((FilePath)AppDomain.CurrentDomain.BaseDirectory).AppendPathSeparator();

	/// <summary>获取应用程序的文件路径（对于 Web 应用程序，返回 <see cref="Empty"/>）。</summary>
	public static readonly FilePath AppPath = Assembly.GetEntryAssembly() != null
		? (FilePath)Assembly.GetEntryAssembly().Location
		: Empty;
					...
					case EndWithLetter:
						if (d) {
							return p.Substring(0,
								(i == 2 && p[1] == Path.VolumeSeparatorChar) || i == 0 ? i + 1 : i);
						}
						else if (c == Path.VolumeSeparatorChar) {
	...
	private static NameList GetDirectories(string path, string[] parts, int partCount) {
		NameList r;
	...
	public bool IsInDirectory(FilePath containingPath, bool rootAsRelative) {
		...
		return v.StartsWith(p, StringComparison.OrdinalIgnoreCase)
			   && (IsDirectorySeparator(p[p.Length - 1]) || (v.Length > p.Length &&
															 IsDirectorySeparator(v[p.Length]) &&
	...
	public bool IsSubPath(bool rootAsRelative) {
		...
		if (_value.Length >= 2 &&
			(_value[1] == Path.VolumeSeparatorChar || _value == ".." || _value.Contains("..."))) {
	...
	public byte[] ReadAllBytes(int maxBytes) {
		...
		using (FileStream s = new(ToFullPath()._value, FileMode.Open, FileAccess.Read,
				   FileShare.ReadWrite)) {
			if (s.CanRead == false) {
#if LIB
					return CollectionHelper.GetEmptyArray<byte>();
	...
	private static class FileSystemEnumerator
	{
		private const int Dir = 1, File = 2;

		private static IEnumerable<string> GetAll(string path, string searchPattern, SearchOption searchOption) {
			return Enumerate(path, searchPattern, searchOption, 0);
		}

		private static IEnumerable<string> GetFile(string path, string searchPattern, SearchOption searchOption) {
			return Enumerate(path, searchPattern, searchOption, File);
		}

		private static IEnumerable<string> GetDir(string path, string searchPattern, SearchOption searchOption) {
			return Enumerate(path, searchPattern, searchOption, Dir);
		}

		private static IEnumerable<string> Enumerate(string path, string searchPattern, SearchOption searchOption,
			int entryType) {
			...
			try {
				...
				NameList subDirs = new();
				do {
					if (findData.cFileName == "." || findData.cFileName == "..") {
						continue;
					}

					if ((findData.dwFileAttributes & (uint)FileAttributes.Directory) ==
				...
				if (subDirs.Count > 0 && searchOption == SearchOption.AllDirectories) {
					foreach (string subdir in subDirs) {
						foreach (string file in Enumerate(subdir, searchPattern, searchOption, entryType)) {
							yield return file;
						}
					}
				}
