...
namespace PDFPatcher.Model
{
	...
	sealed class TextLine : IDirectionalBoundObject
	{
		...
		internal IEnumerable<TextInfo> Texts { get { return _Texts; } }
		/// <summary>获取 <see cref="Texts"/> 内的第一个 <see cref="TextInfo"/>。</summary>
		internal TextInfo FirstText { get { return _Texts[0]; } }
		...
		internal TextLine (TextInfo text) : this() {
			...
			if (text.Text.Length > 2 && text.Region.Height > 0 && text.Region.Width > text.Region.Height * 2) {
				this.Direction = WritingDirection.Hortizontal;
			}
		}

		internal void AddText (TextInfo text) {
			if (this.Direction == WritingDirection.Unknown) {
				var d = this.GetDistance (text.Region);
				this.Direction = InferWritingDirection (d);
				if (this.Direction == WritingDirection.Unknown) {
					d = this.GetDistance (new Bound (text.Region.Center, text.Region.Middle));
					this.Direction = InferWritingDirection (d);
				}
			}
			_Text = null;
			_Texts.Add (text);
			this.Region.Merge (text.Region);
		}
		...
		internal void Merge (TextLine source) {
			_Text = null;
			this.Region.Merge (source.Region);
		...
		internal DistanceInfo GetDistance (Bound other) {
			return this.Region.GetDistance (other, this.Direction);
		}
		...
		private string GetConcatinatedText () {
			...
			if (SuppressTextInfoArrangement == false) {
				if (this.Direction == WritingDirection.Vertical) {
			...
			for (int i = 1; i < l; i++) {
				if (cs > 0) {
					var dx = this.Direction == WritingDirection.Vertical
