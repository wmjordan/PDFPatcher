...
u8* jbig2_encode_generic(struct Pix* const bw, const bool full_headers, const int xres,
    ...
    int* const length)
{
    ...
    const int totalsize = seg.size() + sizeof(pageinfo) + seg2.size() + sizeof(genreg) + datasize + (full_headers ? (sizeof(header) + 2 * endseg.size()) : 0);
    u8* const ret = (u8*)malloc(totalsize);
    int offset = 0;

#define F(x)                             \
    memcpy(ret + offset, &x, sizeof(x)); \
    offset += sizeof(x)
    if (full_headers) {
        F(header);
    }
...
// see comments in .h file
u8* jbig2_encode(int width, int height, int stride, bool zeroIsWhite, uint32_t* const source, int* const length)
{
    u8* ret;
